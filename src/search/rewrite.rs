    ...
    if let Ok(api_key) = env::var("OPENAI_API_KEY") {
        if !api_key.is_empty() {
            ...
            let system_prompt = if is_chinese_query {
                ...
                "你是一个从自然语言查询中提取Rust软件包关键词的专家。请分析用户的问题，识别与Rust生态系统相关的核心概念和功能需求。仅返回逗号分隔的关键词列表。"
            };
    ...
    if let Ok(api_key) = env::var("OPENAI_API_KEY") {
        if !api_key.is_empty() {
            ...
            let system_prompt = if is_chinese_query {
                ...
                "你是一个专门改写Rust软件包查询的助手。分析输入并生成适合在crates.io搜索引擎中使用的关键词。无论输入是关键词还是自然语言问题，都将其转换为相关技术术语和同义词的列表。返回逗号分隔的关键词列表，不要添加解释。"
            };
